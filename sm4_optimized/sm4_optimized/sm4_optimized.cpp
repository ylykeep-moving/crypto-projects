#include <immintrin.h>
#include <iostream>
#include <vector>
#include <chrono>
#include <cstring>
#include <cstdint>

#define ROTL(x,n) (((x) << (n)) | ((x) >> (32 - (n))))

static const uint8_t Sbox[256] = {
    
};


static const uint32_t FK[4] = { 0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc };
static const uint32_t CK[32] = {
    0x00070e15,0x1c232a31,0x383f464d,0x545b6269,0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,
    0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,
    0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,0x30373e45,0x4c535a61,0x686f767d,0x848b9299,
    0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,0x10171e25,0x2c333a41,0x484f565d,0x646b7279
};

inline uint32_t TTransform(uint32_t ka) {
    uint8_t a[4];
    a[0] = (ka >> 24) & 0xff;
    a[1] = (ka >> 16) & 0xff;
    a[2] = (ka >> 8) & 0xff;
    a[3] = ka & 0xff;
    uint32_t b =
        ((uint32_t)Sbox[a[0]] << 24) |
        ((uint32_t)Sbox[a[1]] << 16) |
        ((uint32_t)Sbox[a[2]] << 8) |
        ((uint32_t)Sbox[a[3]]);
    return b ^ ROTL(b, 2) ^ ROTL(b, 10) ^ ROTL(b, 18) ^ ROTL(b, 24);
}

void SM4_KeySchedule(uint32_t SK[32], const uint8_t key[16]) {
    uint32_t MK[4], k[36];
    for (int i = 0; i < 4; ++i) {
        MK[i] = (key[i * 4] << 24) | (key[i * 4 + 1] << 16) |
            (key[i * 4 + 2] << 8) | key[i * 4 + 3];
        k[i] = MK[i] ^ FK[i];
    }
    for (int i = 0; i < 32; ++i) {
        k[i + 4] = k[i] ^ TTransform(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ CK[i]);
        SK[i] = k[i + 4];
    }
}

void SM4_EncryptBlock(const uint32_t sk[32], const uint8_t in[16], uint8_t out[16]) {
    uint32_t x[36];
    for (int i = 0; i < 4; ++i)
        x[i] = (in[i * 4] << 24) | (in[i * 4 + 1] << 16) | (in[i * 4 + 2] << 8) | in[i * 4 + 3];
    for (int i = 0; i < 32; ++i)
        x[i + 4] = x[i] ^ TTransform(x[i + 1] ^ x[i + 2] ^ x[i + 3] ^ sk[i]);
    for (int i = 0; i < 4; ++i) {
        uint32_t t = x[35 - i];
        out[i * 4] = (t >> 24) & 0xff;
        out[i * 4 + 1] = (t >> 16) & 0xff;
        out[i * 4 + 2] = (t >> 8) & 0xff;
        out[i * 4 + 3] = t & 0xff;
    }
}

void EncryptCTR(const uint8_t* input, size_t len, std::vector<uint8_t>& output, const uint8_t key[16], const uint8_t iv[12]) {
    uint32_t sk[32];
    SM4_KeySchedule(sk, key);
    uint8_t counter[16] = { 0 };
    memcpy(counter, iv, 12);
    counter[15] = 1;

    output.resize(len);
    for (size_t i = 0; i < len; i += 16) {
        uint8_t stream[16];
        SM4_EncryptBlock(sk, counter, stream);
        for (int j = 0; j < 16 && i + j < len; ++j)
            output[i + j] = input[i + j] ^ stream[j];
        for (int j = 15; j >= 12; --j)
            if (++counter[j]) break;
    }
}

void EncryptECB_AVX2(const std::vector<uint8_t>& input, std::vector<uint8_t>& output, const uint8_t key[16]) {
    if (input.size() % 64 != 0) {
        std::cerr << "Input size must be a multiple of 64 bytes (4 blocks)\n";
        return;
    }
    uint32_t sk[32];
    SM4_KeySchedule(sk, key);

    output.resize(input.size());

    for (size_t i = 0; i < input.size(); i += 64) {
        for (int j = 0; j < 4; ++j) {
            SM4_EncryptBlock(sk, &input[i + j * 16], &output[i + j * 16]);
        }
    }
}


// 128-bit GF 乘法
void gf128_mul(const uint8_t X[16], const uint8_t Y[16], uint8_t out[16]) {
    uint8_t Z[16] = { 0 };
    uint8_t V[16];
    memcpy(V, Y, 16);

    for (int i = 0; i < 128; ++i) {
        int byte = i / 8, bit = 7 - (i % 8);
        if ((X[byte] >> bit) & 1) {
            for (int j = 0; j < 16; ++j) Z[j] ^= V[j];
        }
        // V = V << 1 (mod poly)
        bool carry = V[0] & 0x80;
        for (int j = 0; j < 15; ++j)
            V[j] = (V[j] << 1) | (V[j + 1] >> 7);
        V[15] <<= 1;
        if (carry) V[15] ^= 0x87;
    }
    memcpy(out, Z, 16);
}

// GHASH(AAD + ciphertext)
void GHASH(const uint8_t H[16], const std::vector<uint8_t>& aad, const std::vector<uint8_t>& cipher, uint8_t out[16]) {
    uint8_t Y[16] = { 0 };
    size_t aad_len = aad.size();
    size_t ct_len = cipher.size();

    size_t total = aad_len + ct_len;
    std::vector<uint8_t> S;
    S.insert(S.end(), aad.begin(), aad.end());
    if (aad_len % 16 != 0)
        S.insert(S.end(), 16 - (aad_len % 16), 0);
    S.insert(S.end(), cipher.begin(), cipher.end());
    if (ct_len % 16 != 0)
        S.insert(S.end(), 16 - (ct_len % 16), 0);

    for (size_t i = 0; i < S.size(); i += 16) {
        for (int j = 0; j < 16; ++j) Y[j] ^= S[i + j];
        gf128_mul(Y, H, Y);
    }

    uint8_t len_block[16] = { 0 };
    uint64_t aad_bits = aad_len * 8;
    uint64_t ct_bits = ct_len * 8;
    for (int i = 0; i < 8; ++i) len_block[7 - i] = (aad_bits >> (i * 8)) & 0xff;
    for (int i = 0; i < 8; ++i) len_block[15 - i] = (ct_bits >> (i * 8)) & 0xff;

    for (int i = 0; i < 16; ++i) Y[i] ^= len_block[i];
    gf128_mul(Y, H, Y);
    memcpy(out, Y, 16);
}

void EncryptGCM(const uint8_t* plaintext, size_t len, const uint8_t key[16], const uint8_t iv[12], const uint8_t* aad, size_t aad_len, std::vector<uint8_t>& ciphertext, uint8_t tag[16]) {
    ciphertext.clear();
    std::vector<uint8_t> aad_vec(aad, aad + aad_len);

    // 1. 生成 H = E_k(0^128)
    uint8_t H[16] = { 0 };
    uint32_t sk[32];
    SM4_KeySchedule(sk, key);
    SM4_EncryptBlock(sk, H, H);

    // 2. 加密 using CTR
    EncryptCTR(plaintext, len, ciphertext, key, iv);

    // 3. 生成 tag = GHASH(AAD || ciphertext || len)
    GHASH(H, aad_vec, ciphertext, tag);

    // 4. tag ^= E_k(IV || 0x00000001)
    uint8_t ctr0[16] = { 0 };
    memcpy(ctr0, iv, 12);
    ctr0[15] = 1;
    uint8_t E0[16];
    SM4_EncryptBlock(sk, ctr0, E0);
    for (int i = 0; i < 16; ++i) tag[i] ^= E0[i];
}

int main() {
    const char* msg = "SM4-GCM Test Message!";
    size_t len = strlen(msg);
    uint8_t key[16] = {
        0x01,0x23,0x45,0x67, 0x89,0xab,0xcd,0xef,
        0xfe,0xdc,0xba,0x98, 0x76,0x54,0x32,0x10
    };
    uint8_t iv[12] = { 0x00,0x01,0x02,0x03, 0x04,0x05,0x06,0x07, 0x08,0x09,0x0a,0x0b };
    const uint8_t aad[] = { 0xa1, 0xa2, 0xa3, 0xa4 };

    std::vector<uint8_t> cipher;
    uint8_t tag[16];

    auto start = std::chrono::high_resolution_clock::now();
    EncryptGCM(reinterpret_cast<const uint8_t*>(msg), len, key, iv, aad, sizeof(aad), cipher, tag);
    auto end = std::chrono::high_resolution_clock::now();

    std::cout << "[+] GCM Cipher: ";
    for (auto c : cipher) std::cout << std::hex << (int)c << " ";
    std::cout << "\n[+] Tag: ";
    for (int i = 0; i < 16; ++i) std::cout << std::hex << (int)tag[i] << " ";
    std::cout << "\n[+] Time: " << std::chrono::duration<double>(end - start).count() << "s\n";
    return 0;
}