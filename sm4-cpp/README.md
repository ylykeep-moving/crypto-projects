...
# SM4 对称加密算法实验 —— CBC 模式加解密实现（C++）

---

## 一、实验目的

1. 理解国密算法 **SM4** 的结构与工作原理；
2. 熟悉对称分组密码的加密模式（如 ECB、CBC）；
3. 通过编程实现 **SM4 算法在 CBC 模式下的加密与解密**；
4. 掌握分组填充（PKCS#7）与 IV 初始化向量的使用。

---

## 二、SM4 算法简介

SM4 是中国商用密码标准中定义的对称加密算法，具有以下特性：

- 块长（Block size）：128 位（16 字节）；
- 密钥长度（Key size）：128 位（16 字节）；
- 轮数（Rounds）：32 轮；
- 算法结构：非 Feistel 型结构，使用 S 盒、线性变换等。

本实验中使用 **CBC 模式（Cipher Block Chaining）**，即每个明文块与上一个密文块进行异或后再加密，第一个块使用 IV（初始向量）。
...
- 编程语言：C++
- 编译器：支持 C++11 及以上的编译器（如 g++、MSVC）
- 标准库：`<vector>`, `<cstring>`, `<iostream>`

---

## 四、CBC 模式工作流程

对于明文分组 $P_1, P_2, \dots, P_n$，密钥 $K$，初始向量 $IV$：
...
其中 $\text{Enc}_K$ 和 $\text{Dec}_K$ 分别为 SM4 的加密和解密操作。
...
- 使用 16 字节对齐的填充（PKCS#7 padding）；
- 每个块加密前先与 IV 或上一个密文块进行 XOR；
- 使用 SM4 核心轮函数 `OneRound` 完成加密。

### 2. 解密函数 `DecryptCBC`

- 每轮解密结果需与上一个密文块异或；
- 解密后移除填充，验证 padding 值是否合法；
- 若格式非法（padding 无效）返回解密失败。

### 3. SM4 核心部分

- 包括 `SetKey()`、`OneRound()`、`RoundF()`、S 盒替换和线性变换；
- 轮密钥通过密钥扩展生成共 32 个；
- 算法使用查表的 Sbox 和固定轮常数 CK。
...
![实验结果](D:\crypto-projects\sm4-cpp\实验结果.png)

### 测试消息：

```text
"Hello SM4 encryption!"
```
...
```
Ciphertext: 53 3c e6 0d b5 79 3b c7 1e b2 33 d3 ...（十六进制密文）
Decrypted: Hello SM4 encryption!
```
...
- **安全性来源**：
  - SM4 算法设计符合国家密码局标准；
  - CBC 模式通过链式依赖增强抗分析能力；
  - 初始向量 IV 每次应重新生成以避免重放攻击。

- **正确性验证**：
  - 解密后与原始明文完全一致；
  - 若 padding 或密文格式错误，程序返回失败提示。

---

## 八、总结与拓展

- 本实验掌握了国密 SM4 算法的基本结构及加解密流程；
- 熟悉了 CBC 模式的数据依赖与实现方式。
